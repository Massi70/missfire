...
class Missfire extends CI_Controller {
	...
	try{
			if( $user_email == false || $user_email == "" ){
				...
				$this->_jsonData['message']="Password Missing";
					...
					$checkUser = $this->webservicesModel->checkUser($user_email,$password);
					$this->_jsonData['status']="SUCCESS";
					if($checkUser['countUser']>0){
						$this->_jsonData['message']="User logged in successfully";
					}else{
			...
			if($user_name == false || $user_name == ""){
				...
				$this->_jsonData['message']="password Missing";
					...
					if($checkUser !=""){
						...
						$this->_jsonData['status']="SUCCESS";
						$this->_jsonData['message']="User Data Inserted In Successfully";
